...
void ControlThread::run()
{
    ...
    demo->Reset();
    sleep(1); //wait 1 second to ensure camera detects new LED sequences assigned to eBug

    SortedID = sortEBugData(this->xBeeNodeList.size(),xTarget,yTarget,this->ebugData);

    QString filename = "/home/ebugcontrol/Desktop/Forcewith4ebugunder20percNoise.txt";
    QFile file(filename);
    ...
    while(1)
    {
        ...
        for(int i = 0;i<xBeeNodeList.size();i++){
            ...
            double noise_percentage = ((qrand()%2)*2 - 1)*0; //noise level 0 percent (FOR DEMONSTRATION)
           ...
           for(int n = 0;n<xBeeNodeList.size();n++) {
               ...
               if(n == i)  continue;
                 ...
                 if(true) {
                    x2 = ebugData[n].position.x()+noise_percentage*ebugData[n].position.x();
                    y2 = ebugData[n].position.y()+noise_percentage*ebugData[n].position.y();
                }
                 else {
...
QList<int> ControlThread::sortEBugData(int nodeSize,double xtarget, double ytarget, QList<EBugData> eBugData)
{
    ...
    for(int a = 0;a<nodeSize;a++) {
        eBugTemp.append(eBugData.at(a));//insert distance at position according to eBug ID
    ...
    for(int i = 0;i<nodeSize;i++) {
        ...
        SortedEbugID.append(eBugTemp.at(closestID).id); //store eBug ID closest to target
